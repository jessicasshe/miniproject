...
public class GameManager {
    ...
    public GameManager()
    {
        ...
        main_character = new MainCharacter(null, 90, home); 
        ...
        office_quiz = new Quiz(input, main_character, ai_robot); //our constructor only has 3 arguments game_manager);
        ...
        factory_quiz = new Quiz(input, main_character, ai_sentinel); //same thing game_manager);
        ...
        hq_quiz = new Quiz(input, main_character, null); //same thing game_manager);
       ...
       // hq_actions.add("Use Memory Chip Contents");
        ...
        headquarters.addAction(new ActionChoice("Hack into a computer", "You begin the hack...", null, main_character));
    ...
    public static int validateIntInput(String prompt, ArrayList<Integer> valid_options)
    {
        int user_choice = 0;
    ...
    {
        // structure: USERNAME | CORRECT ANSWERS(WITHOUT NEW ATTEMPTS) 
        try 
        {
            File leaderboard = new File("leaderboard.txt");
            FileWriter leaderboard_writer = new FileWriter(leaderboard, true);
            leaderboard_writer.write("USERNAME: " +main_character.getUsername()+ " | CORRECT ANSWERS(W/O NEW ATTEMPTS): " +main_character.getScore());
            leaderboard_writer.flush();
            leaderboard_writer.close();
            
            // display current leaderboard
            Scanner leaderboard_reader = new Scanner(leaderboard);
            while(leaderboard_reader.hasNextLine())
            {
                System.out.println(leaderboard_reader.nextLine());
            }
        ...
        {
            System.out.println("Something went wrong while adding score to the leaderboard...");
        }
    }
    ...
    public void showEndingCutscene()
    {
        ...
        printDivider();
        System.out.println("The robots have been deconstructed and the city is safe again. Your father's mission is complete.");
    ...
    private void resetCombatantsForAttempt(Quiz quiz, Enemy enemy){
        main_character.resetHealthToMax();
        
        if(enemy != null){
            enemy.reset_health_to_starting();
        }
        quiz.reset_all_question_attempts();
    }
    
    public void showCombatCutscene(Location curr_location) // use the same local variable
    {
        if(!curr_location.getName().equals("Government Headquarters"))
        {
    ...
    public void run_quiz(){
        
        Quiz quiz = main_character.getCurrentLocation().getQuiz();
        Enemy enemy = main_character.getCurrentLocation().getEnemy();
        
        //int userMaxHP = 100; OLD
        //int enemyMaxHP = enemy.getHealth();

        int attempts = 0;
        boolean all_questions_correct = false;
        while (attempts <= 3){
            System.out.println(CYAN+"THE BATTLE BEGINS"+RESET);
            printDivider();
            resetCombatantsForAttempt(quiz, enemy);

            //quiz.setupCombat(main_character, main_character.getCurentLocation().getEnemy());
            int score = quiz.startQuiz(attempts);
            ...
            if (score == 2){
                ...
                attempts++;
                clearConsole();
                System.out.println("You were defeated..  Remaining quiz attempts: "+(3-attempts));
                //System.out.println(enemy.attack());
                //main_character.takeDamage(enemy.attackDamage);
    ...
    public void showGameOver()
    {
        ...
        System.out.println("You have failed to collect a clue for your mission, succumbing to the dangers of these wandering technologies.");
        System.exit(0);
    }
